...
const getAllStudentsFromDB = async ()=>{
    const excludedFields = ['fullName._id', 'address._id',"_id",'userId','password', 'hobbies','isActive','isDeleted','orders'];
...
const getStudentByIdFromDB = async(id: string)=>{
    const excludedFields = ['fullName._id','address._id','password',"_id",'isDeleted','orders'];
...
const getOrdersByIdFromDB = async(id: string)=>{
    ...
    const excludedFields = ["_id","orders._id",'userId', 'username','password', 'fullName','age', 'email','isActive','hobbies','address'];
...
const updateUserByIdFromDB = async(id: any,body : any)=>{
    const excludedFields = ['fullName._id','address._id','password',"_id",'isDeleted','orders'];
    const result = await usersModel.findOneAndUpdate({userId:id}, {$set: body}).select(`-${excludedFields.join(' -')}`);
    return result
}

// Update User Info by id for Order
